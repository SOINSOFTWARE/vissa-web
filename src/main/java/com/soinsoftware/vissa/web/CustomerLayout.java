...
public class CustomerLayout extends AbstractEditableLayout<Supplier> {
	...
	protected static final Logger log = Logger.getLogger(CustomerLayout.class);
	...
	protected void saveButtonAction(Supplier supplier) {
		...
		if (supplier != null && supplier.getPerson() != null) {
		...
		person = personBuilder.documentType(cbDocumentType.getValue()).documentNumber(txtDocumentId.getValue())
				.name(txtName.getValue()).lastName(lastName).type(PersonType.CUSTOMER).build();
