...
public class ReturnLayoutBk extends VerticalLayout implements View {

	/**
	 * 
	 */
	private static final long serialVersionUID = 5187486966314714738L;
	protected static final Logger log = Logger.getLogger(ReturnLayoutBk.class);
	...
	public ReturnLayoutBk() throws IOException {
		super();
	...
	private void getNextDocumentNumber(DocumentType docType) {
		if (docType != null) {
			...
			/*
			 * ListDataProvider<PaymentDocumentType> payTypeDataProv = new
			 * ListDataProvider<>( paymentDocumentTypeBll.select(documentType));
			 * cbPaymentType.setDataProvider(payTypeDataProv);
			 * cbPaymentType.setItemCaptionGenerator(paymentDocumentType -> { if
			 * (paymentDocumentType != null && paymentDocumentType.getPaymentType() != null)
			 * { return paymentDocumentType.getPaymentType().getName(); } else { return
			 * null; } });
			 */
		} else {
	...
	private void buildProductWindow() {
		...
		try {
			productLayout = new ProductLayout(ELayoutMode.LIST, null);
	...
	/**
	 * Metodo para buscar una factura de venta ya registrada
	 * 
	...
	private void searchInvoice(String documentNumber, EDocumentType eDocType) {
		...
		try {
			...
			if (documentNumber != null && !documentNumber.isEmpty() && documentType != null) {
				document = documentBll.select(documentNumber, documentType);
				if (document != null) {
