...
public class ConciliationLayout extends AbstractEditableLayout<CashRegisterConciliation> {
	...
	private void setFieldValues(CashRegisterConciliation concil) {
		...
		if (concil != null) {
			...
			txtTotalSale.setValue((String.valueOf(concil.getTotalSale())));
			...
			txtSupplierPayment.setValue(String.valueOf(concil.getSupplierPayments()));
	...
	private void voidSaveConciliation(CashRegisterConciliation entity) {
		...
		try {
			...
			entity = conciliationBuilder.person(selectedPerson).conciliationDate(concilitationDate)
					...
					.totalSale(NumericUtil.stringToBigDecimal(txtTotalSale.getValue()))
